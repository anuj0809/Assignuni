...
const PropertyListing = () => {
  ...
  const fetchProperties = async () => {
    try {
      const response = await fetch('https://reunion.vercel.app/api/list-properties');
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(`https://reunion.vercel.app/api/search-properties?searchTerm=${searchTerm}&sort=${sort}`);
      const data = await response.json();
      setProperties(data.data);
    } catch (error) {
